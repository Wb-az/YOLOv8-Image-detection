...
# ## Files
#
# Load an image, download a file, download an IR model, and create a progress bar to show
# download progress.
...
def load_image(path: str) -> np.ndarray:
    ...
    should point to an image file, either a local filename or an url. The image is
...
def download_file(
    ...
    # make chunks bigger so that not too many updates are triggered for Jupyter front-end
    chunk_size = 16384
    ...
    except requests.exceptions.HTTPError as error:  # For error associated with not-200 codes.
        # Will output something like: "404 Client Error: Not Found for url: {url}"
...
def download_ir_model(model_xml_url: str, destination_folder: PathLike = None) -> PathLike:
    ...
    :param destination_folder: Directory where downloaded model xml and bin are saved. \
    If None, model files are saved to the current directory
...
# ### Segmentation
# 
# Define a SegmentationMap NamedTuple that keeps the labels and colormap for a segmentation
# project/dataset. Create CityScapesSegmentation and BinarySegmentation SegmentationMaps.
# Create a function to convert a segmentation map to an RGB image with a colormap, and to show
# the segmentation result as an overlay over the original image.
...
def segmentation_map_to_image(
        result, colormap, remove_holes=False
) -> np.ndarray:
    """
...
# ## Checks and Alerts
# 
# Create an alert class to show stylized info/error/warning messages and a `check_device`
# function that checks whether a given device is available.
...
class NotebookAlert(Exception):
    def __init__(self, message: str, alert_class: str):
        ...
        :param alert_class: The class for styling the message. Options: info, warning,
        success, danger.
        """
...
class DeviceNotFoundAlert(NotebookAlert):
    def __init__(self, device: str):
        """
        Show a warning message about an unavailable device. This class does not check whether
        the device is available, use the `check_device` function to check this. `check_device`
...
def check_openvino_version(version: str) -> bool:
    ...
    if version not in installed_version:
        NotebookAlert(
            ...
            "Please run <span style='font-family:monospace'>pip install --upgrade -r "
